...
pipeline {
  ...
  post {
    always {
      discordSend description: "**Commit** ${GIT_COMMIT}\n**Status**: ${currentBuild.currentResult}\n**Duration**: ${currentBuild.durationString}", footer: "By: ${env.BUILD_USER}\nEmail: ${EMAIL_ADDRESS}\Timestamp: ${BUILD_TIMESTAMP}", link: env.BUILD_URL, result: currentBuild.currentResult, title: "Job: ${JOB_NAME} ${BUILD_DISPLAY_NAME}", webhookURL: env.WEBHOOK_URL
    }
  }
}
